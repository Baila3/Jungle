...
RSpec.describe Product, type: :model do
  describe 'Validations' do
    before do
      ...
      it 'should not save a product with the name field nil' do
          @product = Product.new(name: nil, price: 25.00, quantity: 34, category: @category)
          expect(@product).to_not be_valid
      end

      it 'should not save a product with the quantity field nil' do
        @product = Product.new(name: "Bush", price: 25.00, quantity: nil, category: @category)
        expect(@product).to_not be_valid
      end

      it 'should not save a product with the price field nil' do
        @product = Product.new(name: "Bush", price: "", quantity: 34, category: @category)
        expect(@product).to_not be_valid
      end

      it 'should not save a product with the category field nil' do
